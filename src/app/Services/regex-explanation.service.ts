...
import {RegexPartService} from "./regex-part.service";
...
export class RegexExplanationService {
  ...
  readRegEx(regex: { regex: string; result: any[]; tag: string; hasError: boolean; type: string; label: any; }) {
    const self = this;
    let result: { tag: any; type: any; label: any; regex: any; result: any; hasError: any; from?: any; to?: any; } | null = null;
    let exit = false;
    ...
    while (!exit) {
      ...
      switch(c){
        ...
        case '#':
          ...
          break;
        case '(':
          result =new RegexPart( {type:'class'});
          regex.result.push(result);
  ...
  explain (regex: any) {
    var regexNew = new RegexPartService({obj: {regex: regex}});
    this.readRegEx(regex);
  }

  readEscape(regex: { regex: any; result: any; tag: any; hasError: any; type?: string; label?: any; }) {
    const self = this;
    // @ts-ignore
    let result = new RegexPartService({obj: undefined, type:'escape'});
    regex.result.push(result);
